...
- **Saved projects + repo usage tracking**: `projects` key in `cmux.json` (name + path + optional template ref) PLUS auto-detected repos — `RepoUsageStore` (`Sources/RepoUsageStore.swift`, `~/.cmuxterm/repo-usage.json`) counts git-root-normalized terminal cwd activity (30-min per-repo cooldown) and explicit opens; menus show saved (usage-sorted) then top auto-detected. Surfaces: palette "Project: <name>", Toolbelt submenu, tab-bar folder button (`CmuxSurfaceTabBarBuiltInAction.projects`, in defaults → menu opens tab at repo in current workspace), new-workspace plus-menu "New Workspace in Project", sidebar right-click "Save as Project" (writes global cmux.json via `JSONConfigStore`; name from workspace title, path from focused terminal cwd; same-path save renames). One open path: `openRepoProject(...)` in `Sources/cmuxApp+ForkMenu.swift`. Docs `docs/configuration.md#projects`, schema, tests in `CmuxConfigTests` (+ `RepoUsageStoreTests` same file). Mobile impact: tier 1; "open project from phone" = future tier 3 thin RPC over `openRepoProject`. Logs: `project.save.*` / `project.open` / `projects.loaded` under `agent-templates`. Follow-up not built: per-tab context-menu items (bonsplit submodule owns `TabContextAction`).
