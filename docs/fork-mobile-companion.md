...
- **phone "No response from %@:%d" after scanning**: `handshakeTimedOut` — usually a STALE route. the listener prefers the fixed default port 58465, but a running DEV/tagged cmux squats it, forcing the real app onto a NEW ephemeral port each launch — that's what killed reconnects across restarts. fixed 2026-07-08: John's real app pins `defaults write com.cmuxterm.app mobile.iOSPairingHost.port -int 58500` (settings key `mobile.iOSPairingHost.port`), so persisted phone routes stay valid across promotes/restarts; the phone also refreshes routes from the device registry on reconnect (`DeviceRegistryService`). if pairing fresh, mint a new QR after any mac restart.
